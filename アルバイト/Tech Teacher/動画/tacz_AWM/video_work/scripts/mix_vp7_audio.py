# -*- coding: utf-8 -*-
"""
動画プロジェクト7 Shorts — ナレーション＋BGM版（別mp4）を出力。
- 入力: output/shorts/tacz_vp7_skytree_telop.mp4（テロップ焼き込み済み・
        ゲーム音 +18dB 増幅済み）
       + work/voice/narration_vp7_en.wav（タイミング済み英語ナレーション）
       + input/bgm/Mr_Wick.mp3（BGM）
- ゲーム音とBGMはナレーション中に sidechaincompress でダッキング。
- 映像は再エンコードせずコピー。音声のみ差し替え。
- 出力: output/shorts/tacz_vp7_skytree_narrated.mp4
"""
import os
import subprocess
import sys
...
VIDEO = os.path.join(ROOT, "output", "shorts", "tacz_vp7_skytree_telop.mp4")
NARR = os.path.join(ROOT, "work", "voice", "narration_vp7_en.wav")
BGM = os.path.join(ROOT, "input", "bgm", "Mr_Wick.mp3")
OUT = os.path.join(ROOT, "output", "shorts", "tacz_vp7_skytree_narrated.mp4")
...
# [0:a]=ゲーム音（既に +18dB 増幅済み）
# [1:a]=ナレーション（+6dB で前に出す。3分岐: 合成用 + 2系統のダッキング用）
# [2:a]=BGM（Mr_Wick mean -14.8dB → -16dB で背景化、さらにダッキング）
FILTER = (
    "[2:a]atrim=0:17,asetpts=PTS-STARTPTS,aresample=48000,"
